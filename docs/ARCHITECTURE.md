...
- **Polyglot by design.** Every backend service is a different enterprise
  ...
  confined to the SvelteKit frontend.
...
- **Internal gRPC** — migrate gateway→service calls from HTTP to gRPC.
...
SvelteKit (carried from v1). It only talks to the Go gateway. Frontend types
are generated from the gateway's OpenAPI spec (`openapi-typescript` / Orval),
